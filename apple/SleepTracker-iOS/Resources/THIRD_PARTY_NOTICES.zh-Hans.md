...
Circadia 在中国大陆等部分地区采用 **Qwen 系列**开放模型权重，由 **阿里巴巴通义实验室（Tongyi Lab, Alibaba Cloud / Alibaba Group）** 公开发布。

- 架构与权重 © Alibaba Group / Tongyi Lab；
- 使用受 **Apache License 2.0**（Qwen 主线开源许可）约束，部分变体可能附加额外使用条款；以官方仓库与模型卡为准；
- Circadia 不再分发原始权重；如你选择下载，仍以阿里巴巴及上游发布方的适用条款为准；
- 详见 <https://github.com/QwenLM/Qwen3> 及对应 Hugging Face 模型卡。

如 Circadia 随附基于 Qwen 的 LoRA 微调适配器（adapter），相关适配器作为衍生作品同样受其上游许可证约束。
...
|---|---|---|---|---|---|
| Gemma-2-2B (Instruct, 4-bit) | Google DeepMind / Hugging Face | 海外地区 Sleep AI 推理 | Gemma Terms of Use | 否（推理时加载 LoRA 适配器） | 不再分发原始权重 |
| Qwen3-1.7B (Instruct, 4-bit) | 阿里巴巴通义实验室 / Hugging Face | 中国大陆等地区 Sleep AI（Instant 档） | Apache 2.0 | 否（推理时加载 LoRA 适配器） | 不再分发原始权重 |
| Qwen3-4B (Instruct, 4-bit) | 阿里巴巴通义实验室 / Hugging Face | 中国大陆等地区 Sleep AI（Pro 档） | Apache 2.0 | 否（推理时加载 LoRA 适配器） | 不再分发原始权重 |
| Sleep-AI LoRA adapter（Gemma 系） | BIRI GA（基于 Gemma 微调） | 健康话题约束 | Gemma Terms of Use（衍生作品） | 是 | 由 Circadia 训练并随 App 分发 |
| Sleep-AI LoRA adapter（Qwen 系） | BIRI GA（基于 Qwen3 微调） | 健康话题约束 | Apache 2.0（衍生作品） | 是 | 由 Circadia 训练并随 App 分发 |
